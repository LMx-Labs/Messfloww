...
async function runValidationTests() {
  ...
  {
    const rtdb = new MockRtdbDatabase() as any;
    ...
    const acquireRes = await acquireRtdbLeases(rtdb, cart, intentId, 120000);
    ...
    assert(postAcquireStock.activeLeases[intentId]?.qty === 3, 'Active lease registered with intentId key');

    // Step 2: Commit Lease (Phase 2)
    await commitRtdbLeases(rtdb, cart, intentId);
  ...
  console.log('\n--- TEST 2: Lease Compensation on Payment Abort (FM-01 Fix) ---');
  {
    ...
    // Acquire lease
    await acquireRtdbLeases(rtdb, cart, intentId, 120000);
    ...
    await releaseRtdbLeases(rtdb, cart, intentId);
  ...
  console.log('\n--- TEST 3: Inline OCC Lazy Lease Reclamation (Zero Sweeper Delay) ---');
  {
    ...
    const res = await acquireRtdbLeases(rtdb, newCart, newIntentId, 120000);
    ...
    assert(finalStock.activeLeases[newIntentId]?.qty === 3, 'New lease successfully acquired');
  }
  ...
  console.log('\n--- TEST 4: Multi-Item Atomic Rollback on Partial Cart Failure ---');
  {
    ...
    const res = await acquireRtdbLeases(rtdb, cart, intentId, 120000);
  ...
  {
    const db = new MockFirestoreDatabase() as any;
    const rtdb = new MockRtdbDatabase() as any;

    // Case 5A: Wallet WAS debited, but dispatch crashed (FM-03 scenario) -> Forward Recovery
    const intentA: TransactionIntentDocument = {
    ...
    await reconcileIntent(db, rtdb, 'INTENT-RECON-FWD', intentA);
    ...
    const intentB: TransactionIntentDocument = {
    ...
    await reconcileIntent(db, rtdb, 'INTENT-RECON-BACK', intentB);
  ...
  console.log('\n--- TEST 6: Idempotent Replay on Lost Response (FM-04 Fix) ---');
  {
    ...
    const committedIntent: TransactionIntentDocument = {
    ...
    const existing = intentSnap.data() as TransactionIntentDocument;
  ...
  console.log('\n--- TEST 7: Unknown Outcome Network Drop & Forward Recovery ---');
  {
    ...
    const droppedIntent: TransactionIntentDocument = {
    ...
    await commitRtdbLeases(rtdb, existing.cart, existing.intentId);

...
async function executeWithFailureInjection(db, rtdb, params) {
    var _a;
    ...
    try {
        ...
        // Acquire RTDB leases
        const leaseRes = await (0, index_1.acquireRtdbLeases)(rtdb, cart, intentId, leaseDurationMs);
        if (!leaseRes.success) {
        ...
        await db.runTransaction(async (tx) => {
            ...
            const bal = studentSnap.exists ? ((_a = studentSnap.data()) === null || _a === void 0 ? void 0 : _a.balance) || 0 : 0;
            if (bal < totalPrice)
        ...
        try {
            ...
            if (intentSnap.exists) {
                const doc = intentSnap.data();
                if ((_a = doc.journal) === null || _a === void 0 ? void 0 : _a.firestoreWalletDebited) {
...
async function runAdversarialValidation() {
    ...
    // Point 1: Before intent creation
    {
        ...
        const intent = (_a = db.collections['transaction_intents']) === null || _a === void 0 ? void 0 : _a['INT-FAIL-01'];
        const bal = db.collections['students']['S01'].balance;
        report('Point 1 (Before intent creation): Clean abort, zero side effects', res.completed === false && stock.stock === 10 && stock.reserved === 0 && !intent && bal === 500);
    }
    // Point 2: After intent creation, before RTDB lease acquisition
    {
        ...
        const intent = (_b = db.collections['transaction_intents']) === null || _b === void 0 ? void 0 : _b['INT-FAIL-02'];
        const bal = db.collections['students']['S02'].balance;
        report('Point 2 (After intent, before lease): Intent CANCELLED, stock untouched, balance untouched', res.completed === false && stock.stock === 5 && stock.reserved === 0 && (intent === null || intent === void 0 ? void 0 : intent.state) === 'CANCELLED' && bal === 200);
    }
    // Point 3: After inventory reservation, before financial operation
    {
        ...
        const intent = (_c = db.collections['transaction_intents']) === null || _c === void 0 ? void 0 : _c['INT-FAIL-03'];
        const bal = db.collections['students']['S03'].balance;
        report('Point 3 (After reservation, before financial): Catch handler released lease, restored stock', res.completed === false && stock.stock === 10 && stock.reserved === 0 && (intent === null || intent === void 0 ? void 0 : intent.state) === 'CANCELLED' && bal === 300);
    }
    // Point 4: During financial operation abort (Insufficient Balance)
    {
        ...
        const intent = (_d = db.collections['transaction_intents']) === null || _d === void 0 ? void 0 : _d['INT-FAIL-04'];
        const bal = db.collections['students']['S04'].balance;
        report('Point 4 (Financial abort - Insufficient balance): Catch handler compensates, stock restored', res.completed === false && stock.stock === 10 && stock.reserved === 0 && (intent === null || intent === void 0 ? void 0 : intent.state) === 'CANCELLED' && bal === 20);
    }
    // Point 5: After financial operation, before RTDB order creation (Crash in between)
    {
        ...
        const intent = (_e = db.collections['transaction_intents']) === null || _e === void 0 ? void 0 : _e['INT-FAIL-05'];
        ...
        report('Point 5 (After financial debit): Catch handler executed forward recovery, created order', res.recovered === true && stock.stock === 9 && stock.reserved === 0 && (intent === null || intent === void 0 ? void 0 : intent.state) === 'COMMITTED' && bal === 150 && order !== undefined);
    }
    // Point 6: Before order creation (Simulating hard container crash where catch handler does NOT run)
    {
        ...
        await (0, index_1.acquireRtdbLeases)(rtdb, [{ id: 6, name: 'Item 6', qty: 1 }], 'INT-HARD-CRASH-06', 120000);
        ...
        await (0, index_1.reconcileIntent)(db, rtdb, 'INT-HARD-CRASH-06', intent6);
        ...
        report('Point 6 (Hard crash before order creation): Sweeper performed forward recovery, order created', intentAfterSweep.state === 'COMMITTED' && stockAfterSweep.stock === 9 && stockAfterSweep.reserved === 0 && orderAfterSweep !== undefined);
    }
    // Point 7: After order creation, before commitRtdbLeases
    {
        ...
        const intent = (_f = db.collections['transaction_intents']) === null || _f === void 0 ? void 0 : _f['INT-FAIL-07'];
        const order = rtdb.data['active_orders/ORD-INT-FAIL-07'];
        report('Point 7 (After order creation): Forward recovery in catch committed leases and finalized intent', res.recovered === true && stock.stock === 9 && stock.reserved === 0 && (intent === null || intent === void 0 ? void 0 : intent.state) === 'COMMITTED' && order !== undefined);
    }
    // Point 8: During compensation (Catch handler crashes mid-rollback)
    {
        ...
        report('Point 8A (Compensation handler crash): Incomplete state captured in durable store', res.completed === false && intentBefore.state === 'RESERVED' && stockBefore.reserved === 2);
        ...
        await (0, index_1.reconcileIntent)(db, rtdb, 'INT-FAIL-08', intentBefore);
        const intentAfter = db.collections['transaction_intents']['INT-FAIL-08'];
        const stockAfter = rtdb.data['menu_stock/8'];
        report('Point 8B (Sweeper recovers Point 8 crash): Expired lease swept, stock restored to 10', intentAfter.state === 'CANCELLED' && stockAfter.stock === 10 && stockAfter.reserved === 0);
    }
    // Point 9: During recovery (Forward recovery order creation fails on RTDB -> fallback to refund)
    {
        ...
        await (0, index_1.reconcileIntent)(db, rtdb, 'INT-REC-09', brokenIntent);
        ...
        const refundDoc = ledgerDocs.find((l) => l.type === 'refund' && l.intentId === 'INT-REC-09');
        report('Point 9 (Recovery RTDB failure fallback): Student fully refunded, ledger logged, leases released', intentAfter.state === 'CANCELLED' && studentBal === 200 && userBal === 200 &&
    ...
    // Point 10: After recovery begins but before completion (Sweeper crashes after dispatch, before state update)
    {
        ...
        await (0, index_1.reconcileIntent)(db, rtdb, 'INT-REC-10', intent10);
        const intentFinal = db.collections['transaction_intents']['INT-REC-10'];
        const stockFinal = rtdb.data['menu_stock/10'];
        report('Point 10 (Re-entrant recovery after mid-sweep crash): Successfully finalized to COMMITTED without duplicating order', intentFinal.state === 'COMMITTED' && stockFinal.reserved === 0 && stockFinal.stock === 9);
    }
    // ════════════════════════════════════════════════════════════════════════════
    ...
    // Scenario: Client submits order -> Server finishes Phase 4 -> TCP Connection drops -> Client retries
    {
        ...
        report('Phase 4B: Replay detected committed state without re-executing', (secondRunReceipt === null || secondRunReceipt === void 0 ? void 0 : secondRunReceipt.idempotentReplay) === true && secondRunReceipt.orderId === firstRun.orderId);
    ...
    // Two identical requests arrive concurrently before either has written to transaction_intents.
    {
        ...
        const acquire1 = await (0, index_1.acquireRtdbLeases)(rtdb, [{ id: 60, name: 'Item 60', qty: 1 }], sharedKey, 120000);
        const acquire2 = await (0, index_1.acquireRtdbLeases)(rtdb, [{ id: 60, name: 'Item 60', qty: 1 }], sharedKey, 120000);
        ...
        report('VULNERABILITY DETECTED: TOCTOU race on non-atomic intent creation allows duplicate lease decrement', bothSawNonExistent && acquire1.success && acquire2.success && stockDoubleDecremented && reservedIsTwo, 'When two concurrent requests hit with identical key before intent is written, both pass non-transactional get(), acquiring dual stock decrements!');
    }
    ...
    // Client A resumes and commits!
    {
        ...
        await (0, index_1.acquireRtdbLeases)(rtdb, cart, clientAIntent, 1000); // 1-second TTL
        ...
        const acquireB = await (0, index_1.acquireRtdbLeases)(rtdb, cart, clientBIntent, 120000);
        ...
        await (0, index_1.commitRtdbLeases)(rtdb, cart, clientAIntent);
        // 5. Client B now finishes and calls commitRtdbLeases!
        await (0, index_1.commitRtdbLeases)(rtdb, cart, clientBIntent);
        ...
        report('VULNERABILITY DETECTED: Lack of Fencing Token allows delayed client to commit after lease was reclaimed', clientAHeldStock && clientBHeldStock && finalStock.stock === 0 && finalStock.reserved === 0, 'If lease expires during Phase 3, stock is reclaimed and sold to Client B. Client A finishes and commits without lease ownership validation — 1 physical item sold to 2 students!');
    }
    ...
    // At the exact same moment, the client retries!
    {
        ...
        await (0, index_1.commitRtdbLeases)(rtdb, raceIntent.cart, raceIntent.intentId);
        ...
        report('Deterministic Path Prevents Duplicate Orders in Sweeper vs Client Race', activeOrdersCount === 1 && sweeperDispatched && clientDispatched && finalStock.stock === 9 && finalStock.reserved === 0, 'Because orderId is deterministic (anchored to intentId), dual dispatch collides harmlessly at the same RTDB key.');
    }
    ...
    // Client 1 requests [Item A, Item B]. Client 2 requests [Item B, Item A].
    {
        ...
        report('RTDB OCC Transaction Semantics Prevent Locking Deadlocks on Multi-Item Carts', true, 'Transactions commit optimistically per key without cross-key mutex locks.');
    }
    // ════════════════════════════════════════════════════════════════════════════
    ...
    // PROOF 1: Atomic Intent Gate Eliminates Failure A (TOCTOU Race)
    {
        ...
        async function workerSim(workerId) {
            ...
            if (isNewIntent) {
                ...
                const leaseRes = await (0, index_1.acquireRtdbLeases)(rtdb, initialIntent.cart, sharedKey, 120000);
        ...
        report('Proof 1: Atomic docRef.create() eliminates TOCTOU race (Exactly 1 winner, 1 blocked, single lease decrement)', winnerCount === 1 && blockedCount === 1 && stockAfter.stock === 9 && stockAfter.reserved === 1);
    }
    // PROOF 2: Monotonic Fencing Token & Compensating Auto-Refund Eliminates Failure B (Lease Expiry Race)
    {
        ...
        const acquireA = await (0, index_1.acquireRtdbLeases)(rtdb, cart, clientAIntent, 1000, fenceA);
        if (!acquireA.success)
            throw new Error('Setup failed: acquireA should succeed');
        ...
        const acquireB = await (0, index_1.acquireRtdbLeases)(rtdb, cart, clientBIntent, 120000, fenceB);
        // 4. Client A wakes up and attempts commitRtdbLeases with its stale fenceToken (fenceA)
        const commitAttemptA = await (0, index_1.commitRtdbLeases)(rtdb, cart, clientAIntent, fenceA);
        ...
        if (!commitAttemptA.success && commitAttemptA.reason === 'LEASE_FENCING_REVOKED') {
            ...
            await (0, index_1.refundStudentWallet)(db, intentADoc, 'Fencing token revoked: lease expired and reclaimed');
        ...
        const commitAttemptB = await (0, index_1.commitRtdbLeases)(rtdb, cart, clientBIntent, fenceB);
        ...
        report('Proof 2A: commitRtdbLeases rejected stale Client A commit via fencing token', commitAttemptA.success === false && commitAttemptA.reason === 'LEASE_FENCING_REVOKED');
        report('Proof 2B: Delayed Client A received full wallet balance refund upon fencing rejection', clientARefunded && balanceA === 500 && intentAFinal.state === 'CANCELLED');
        report('Proof 2C: Active Client B committed successfully with active fence token', acquireB.success && commitAttemptB.success === true);
        report('Proof 2D: Invariants preserved: Zero overselling (stock 0, reserved 0), exactly 1 physical sale', stockFinal.stock === 0 && stockFinal.reserved === 0 && balanceB === 400);
    }
    // ════════════════════════════════════════════════════════════════════════════
    // SUMMARY REPORT
    // ════════════════════════════════════════════════════════════════════════════
...
//# sourceMappingURL=adversarialBreakTest.js.map